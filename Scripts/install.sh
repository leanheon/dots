...
EOF
    ...
    # Hyde-Caelestia Integration: caelestia-shell 설정
    if [ -f "${scrDir}/integration/setup_caelestia.sh" ]; then
        cat <<"EOF"

 _   _           _        ___           _           _   _       
| | | |_   _  __| | ___  / __\__ _  ___| | ___  ___| |_(_) __ _ 
| |_| | | | |/ _` |/ _ \/ /  / _` |/ _ \ |/ _ \/ __| __| |/ _` |
|  _  | |_| | (_| |  __/ /__| (_| |  __/ |  __/\__ \ |_| | (_| |
|_| |_|\__, |\__,_|\___\____/\__,_|\___|_|\___||___/\__|_|\__,_|
       |___/                                                   

EOF
        print_log -g "Hyde-Caelestia" " :: " "Setting up caelestia-shell integration..."
        "${scrDir}/integration/setup_caelestia.sh"
        
        # 테마 동기화 실행
        if [ -f "${scrDir}/integration/theme_bridge.sh" ]; then
            print_log -g "Hyde-Caelestia" " :: " "Synchronizing themes..."
            "${scrDir}/integration/theme_bridge.sh"
        fi
        
        print_log -g "Hyde-Caelestia" " :: " "Integration completed!"
    fi
fi
